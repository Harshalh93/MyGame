...
export default class Lv2 extends Phaser.Scene {
  ...
  create(){
    ...
    var rb = this.textures.get('robot')
    var r = rb.getFrameNames()
    console.log(r)
    ...
    this.player= this.physics.add.sprite(100,150,'robot', r[0]).setScale(0.45)
    ...
    for(var i=0;i<10;i++){
      ...
      var v = [x, x+500]
      this.coins.create(v[rnd], y-100,'all',b[32] ).setScale(0.5).play('bronze')
      y = y - 300
    } 
    this.score = 0
  ...
  update() {
    ...
    if (this.pointer.isDown){
     ...
     if(px>this.scale.width/2){
       this.player.play('walks').setFlipX(false)
        this.player.setVelocityY(-200)
        this.player.setVelocityX(100)
     } else if (px<this.scale.width/2) {
       this.player.play('walks').setFlipX(true)
       this.player.setVelocityY(-200)
       this.player.setVelocityX(-100)
      }
